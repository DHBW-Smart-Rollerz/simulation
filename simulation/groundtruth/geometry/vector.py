...
class Vector:
    ...
    Composition with shapely enables to use their powerful operations in combination with
    ...
    def __init__(self, *args, **kwargs):
        ...
        with suppress(Exception):
            self._point = shapely.geometry.point.Point(*args)
            return
    ...
    def rotated(self, arg: Union[float, Quaternion]):
        ...
        if isinstance(arg, Quaternion):
            ...
            return self.__class__(
                c * self.x - s * self.y, s * self.x + c * self.y, self.z
            )
