...
from simulation.groundtruth.basics.init_options import InitOptions
from simulation.groundtruth.basics.save_options import SaveOptions
from simulation.groundtruth.geometry import Line, Transform
...
class CustomSection(RoadSection, InitOptions, SaveOptions):
    ...
    def split_by(
        ...
        remaining_custom_section = CustomSection(
            middle_line_points=[
                tf * p
                for p in Line.cut(self.middle_line, x_buffered_end)[1].get_points()
            ]
        )
