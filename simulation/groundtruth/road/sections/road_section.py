...
from simulation.groundtruth.geometry import (
    Line,
    Point,
    Polygon,
    Pose,
...
from simulation.groundtruth.road.sections.speed_limit import SpeedLimit
...
class MarkedLine(Line):
    ...
    def from_line(cls, line: Line, style, prev_length=0):
        ...
        m._linestring = line._linestring
        return m
...
class RoadSection(Transformable):
    ...
    def get_beginning(self) -> Tuple[Pose, float]:
        ...
        """
        pose = Transform([0, 0], math.pi) * self.middle_line.interpolate_pose(
            arc_length=0
        )
    ...
    def fit_ending(
        ...
        from .bezier_curve import (
            CubicBezier,  # Import here to prevent cyclic dependency!
        )
