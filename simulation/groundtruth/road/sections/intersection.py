...
from simulation.groundtruth.geometry import Line, Point, Polygon, Pose, Vector
...
from simulation.groundtruth.road.sections.surface_marking import SurfaceMarkingRect
from simulation.groundtruth.road.sections.traffic_sign import TrafficSign
...
class Intersection(RoadSection):
    ...
    def __post_init__(self):
        ...
        assert (-1 * self.w + self.v).y > (-1 * self.u).y and self.z.x > (
            self.x - self.u
        ).x
    ...
    def get_beginning(self) -> Tuple[Pose, float]:
        return (
            Pose(self.transform * Point(0, 0), self.transform.get_angle() + math.pi),
            0,
        )
    ...
    def get_ending(self) -> Tuple[Pose, float]:
        ...
        return (
            Pose(self.transform * end_point, self.transform.get_angle() + end_angle),
            0,
        )
    ...
    def _get_intersection_surface_markings(self) -> List[SurfaceMarkingRect]:
        ...
        if self.rule == Intersection.EQUAL and self.turn == Intersection.STRAIGHT:
            ...
            self.rule == Intersection.PRIORITY_STOP
            and self.turn == Intersection.STRAIGHT
        ):
