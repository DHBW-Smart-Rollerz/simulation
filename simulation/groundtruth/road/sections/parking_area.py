...
from simulation.groundtruth.geometry import Line, Point, Polygon, Transform
...
from simulation.groundtruth.road.sections.road_element import RoadElementRect
from simulation.groundtruth.road.sections.road_section import MarkedLine
from simulation.groundtruth.road.sections.transformable import Transformable
...
class ParkingSpot(RoadElementRect):
    ...
    def lines(self) -> List[MarkedLine]:
        ...
        if self._side == ParkingLot.LEFT_SIDE or self.kind == self.BLOCKED:
            ...
            lines.append(
                MarkedLine.from_line(left_border, RoadSection.SOLID_LINE_MARKING)
            )
            lines.append(
                MarkedLine.from_line(right_border, RoadSection.SOLID_LINE_MARKING)
            )
        return lines
